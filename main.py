...
def cross_reference():
    if len(analysis_results) < 2:
        ui.notify("Need at least 2 documents.", type="warning")
        return
    patient_map = {}
    inconsistencies = []
    for res in analysis_results:
        ...
        if pid_match and name_match:
            ...
            if pid in patient_map and patient_map[pid] != name:
                inconsistencies.append(f"ID {pid} has conflicting names: {patient_map[pid]} vs {name}")
            else:
    ...
    if inconsistencies:
        ...
        if analysis_results:
            analysis_results[0]["suspicious_flags"] = analysis_results[0].get("suspicious_flags", []) + ["cross-reference inconsistency"]
            update_results_panel()
    else:
        ui.notify("All patient IDs match.", type="positive")
...
with ui.column().classes("w-full items-center p-6"):
    ui.upload(on_upload=handle_upload, multiple=True, auto_upload=True, label="📄 Upload .txt files").props("accept=.txt").classes("w-full max-w-3xl")
    with ui.row().classes("justify-center gap-4 my-4"):
        ui.button("🔄 Cross-Reference", on_click=cross_reference, color="orange")
        ui.button("🗑️ Clear All", on_click=clear_all, color="red")
    ui.label("📋 Results").classes("text-xl font-bold mt-4")
