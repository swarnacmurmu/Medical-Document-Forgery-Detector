...
def generate_forged(genuine):
    ...
    # 3. Mismatched treatment
    if random.choice([True, False]):
        forged["Treatment"] = random.choice(treatments)
        flags.append("treatment_mismatch")
    # 4. Ghost patient (name mismatch)
    if random.choice([True, False]):
        forged["Patient Name"] = f"{random.choice(first_names)} {random.choice(last_names)}"
        flags.append("patient_name_mismatch")
    # 5. ID mismatch
    if random.choice([True, False]):
        forged["Patient ID"] = f"P{random.randint(10000, 99999)}"
        flags.append("patient_id_mismatch")
    return forged, flags
...
for i in range(NUM_FILES):
    ...
    forged, flags = generate_forged(genuine)
